                                 ...
                                 mca_coll_ucc_req_t *coll_req)
{
    ...
    if (comm_rank == root) {
        max_rcounts = rcounts[0];
        for (i = 1; i < comm_size; ++i) {
            if (max_rcounts < rcounts[i]) {
                max_rcounts = rcounts[i];
            }
        }
        if ((is_inplace || ompi_datatype_is_contiguous_memory_layout(sdtype, scount)) &&
            ompi_datatype_is_contiguous_memory_layout(rdtype, max_rcounts)) {
    ...
    if (UCC_OK != status) {
        if (status == UCC_ERR_NOT_SUPPORTED) {
            UCC_VERBOSE(5, "gatherv: non-contiguous datatype detected across ranks");
        } else if (status == UCC_ERR_INVALID_PARAM) {
            UCC_VERBOSE(5, "gatherv: asymmetric datatypes or memory types detected");
        }
        goto fallback;
    }
